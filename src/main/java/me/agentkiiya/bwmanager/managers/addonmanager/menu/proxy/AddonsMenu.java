package me.agentkiiya.bwmanager.managers.addonmanager.menu.proxy;

import com.google.gson.JsonObject;
import com.tomkeuper.bedwars.proxy.BedWarsProxy;
import me.agentkiiya.bwmanager.Manager;
import me.agentkiiya.bwmanager.managers.addonmanager.menu.GUIHolder;
import me.agentkiiya.bwmanager.managers.addonmanager.addons.CachedAddon;
...
public class AddonsMenu implements GUIHolder {
    ...
    public void addContents() {
        ...
        if (option.startsWith("--")) {
            switch (option.replace("--", "").toLowerCase()) {
                case "loaded":
                    ...
                    Bukkit.getScheduler().runTaskLater(Manager.getInstance(), () -> addons.set(CachedAddon.getCachedLoadedAddons()), 30L);
                    break;
                case "unloaded":
                    object.addProperty("addons-action", "get-unloaded-addons");
                    BedWarsProxy.getRedisConnection().sendMessage(object, "addon-manager");
                    Bukkit.getScheduler().runTaskLater(Manager.getInstance(), () -> addons.set(CachedAddon.getCachedUnloadedAddons()), 30L);
                    break;
                case "registered":
                    object.addProperty("addons-action", "get-addons");
                    BedWarsProxy.getRedisConnection().sendMessage(object, "addon-manager");
                    Bukkit.getScheduler().runTaskLater(Manager.getInstance(), () -> addons.set(CachedAddon.getCachedAddons()), 30L);
                    break;
                default:
                    close = true;
            ...
            object.addProperty("addons-action", "get-addons");
            BedWarsProxy.getRedisConnection().sendMessage(object, "addon-manager");
            Bukkit.getScheduler().runTaskLater(Manager.getInstance(), () -> addons.set(CachedAddon.getAddonsByAuthor(option)), 30L);
        }
        ...
        closeMeta.setLore(Utility.cList(List.of("&7Close this inventory")));
